...
const getExecutable = (): string[] => {
	switch (process.platform) {
		case 'win32':
			return [os.homedir() + '/AppData/Local/Programs', 'c:/Program Files', 'c:/Program Files (x86)'].map((dir) =>
				path.join(dir, 'Microsoft VS Code', 'Code.exe')
			);
		case 'darwin':
			return [path.join('/Applications', 'Visual Studio Code.app', 'Contents', 'MacOS', 'Electron')];
		case 'linux':
...
export const startRun = (name: string) => {
	...
	const sp = spawn(`${executable}`, [`--extensions-dir=${extensionsDir}`, `--user-data-dir=${userDataDir}`], {
		detached: true,
		stdio: 'ignore',
	});
	sp.unref();
...
export const startInteractiveRun = async () => {
	...
	try {
		...
		configs = dirents.filter((_dirent) => _dirent.isDirectory()).map((_dirent) => ({ name: _dirent.name }));
	} catch (e) {
		configs = [];
	}
